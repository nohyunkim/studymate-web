...
class RequestContext:
    ...
    def redirect(self, location, status=302):
        response = Response.new("", status=status, headers={"Location": location})
    ...
    def render(self, template_name, **context):
        ...
        response = Response.new(rendered, headers={"Content-Type": "text/html; charset=utf-8"})
        return self.finalize(response)
    ...
    def text(self, text, status=200):
        response = Response.new(text, status=status, headers={"Content-Type": "text/plain; charset=utf-8"})
        return self.finalize(response)
    ...
    def finalize(self, response):
        ...
        if self.session.get("user_id") or self.session.get("_csrf_token") or self.session.get("flashes"):
            ...
            response.headers.set("Set-Cookie", build_set_cookie(SESSION_COOKIE_NAME, token, secure=secure))
        elif self.mutated:
            response.headers.set("Set-Cookie", delete_cookie(SESSION_COOKIE_NAME, secure=secure))
        return response
